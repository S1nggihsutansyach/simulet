...
<div class="card">
    ...
    <div class="card-body">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            <form action="{{ route('tambahkategorilayanan.admin') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col">
        ...
        <table class="table mt-3">
            ...
            <tbody>
                @foreach ($katlayanan as $no => $data)
                <tr>
                    <th scope="row">{{ $katlayanan->firstItem() + $no }}</th>
                    <td><img style="width: 70px" src="{{ url('/storage/'.$data->image) }}" alt=""></td>
                    <td>{{ $data->name }}</td>
                    <td>
                        <a name="" id="" class="btn btn-sm btn-warning" href="{{ route('tampilubahkategorilayanan.admin',['slug'=>$data->slug]) }}" role="button"><i class="fa fa-pen"></i></a>
                        <form class="d-inline" action="{{ route('hapuskategorilayanan.admin',['slug'=>$data->slug]) }}" method="post">
                            @csrf
                            @method('delete')
